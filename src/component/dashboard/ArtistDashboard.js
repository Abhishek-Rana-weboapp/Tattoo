...
export default function ArtistDashboard() {
  ...
  const { isVisible, setIsVisible, alert , setAlert , setAlertMessage, setUpdateAppointment } = useContext(UserContext);
    ...
    if(step === 1){
      if(selectedArtist){
        if(selectedYes.length !== 0){
          updateField(selectedClient?.id , "ArtistPiercerNames" , selectedArtist).then(res=>console.log(res))
          setStep(2)
        }else{
          updateField(selectedClient?.id , "ArtistPiercerNames" , selectedArtist).then(res=>console.log(res))
          navigate("/billing")
    ...
    { step === 0 &&
      <div className="flex flex-col justify-center gap-2 items-center">
        ...
        <div className="flex justify-center">
          <button className="yellowButton rounded-xl py-2 px-5 font-bold" onClick={handleNext}>Next</button>
          </div>
        </div>
      }
      ...
      { step === 1 && <div className="flex flex-col justify-center gap-2 items-center">
        ...
        <button className="yellowButton rounded-xl py-2 px-5 font-bold" onClick={handlePrev}>Back</button>
          <button className="yellowButton rounded-xl py-2 px-5 font-bold" onClick={handleNext}>Next</button>
          </div>
      </div>}
              ...
              return (
                <div className="flex flex-col gap-1 text-white w-full ">
                  ...
                  <div className="flex gap-2 w-full items-center justify-center">
                    <input type="checkbox" className="w-6 h-6" value={acknowledgement} onChange={(e)=>setAcknowledgement(e.target.checked)}/>
                    <label className="font-bold text-white" >I understand the Medical History of the client</label>
                  </div>
          </div>
          <div className="flex justify-center gap-4">
          <button className="yellowButton rounded-xl py-2 px-5 font-bold" onClick={handlePrev}>Back</button>
