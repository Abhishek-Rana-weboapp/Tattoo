...
export default function MedicalFormLayout({ children, title, about }) {
  ...
  return (
    <div className="w-full sm:w-3/4  rounded-md p-4 md:w-2/3 lg:w-1/2 xl:w-1/2  flex flex-col gap-3 " style={{height:"100dvh"}}>
      ...
      <h1 className="font-bold text-xl  md:text-4xl text-white  uppercase text-center">{title}</h1>
      <div className="w-full bg-gray-800 backdrop-blur bg-opacity-50 flex-1 p-2 rounded-md">{children}</div>
    </div>
  );
}
