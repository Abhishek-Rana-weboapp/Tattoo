...
function HoldHarmlessAgreement() {
  const {harmlessagreement,setharmlessagreement} = React.useContext(UserContext);
  ...
  const handleNameChange = (e) => {
    setharmlessagreement({
      ...harmlessagreement, name:e.target.value
    })
  };

  const handleInitialsChange = (e) => {
    setharmlessagreement({
      ...harmlessagreement,initials:e.target.value
    })
  };
  ...
  const handleSaveSignature = () => {
   ...
   setharmlessagreement({
      ...harmlessagreement, signatureurl:dataUrl
    })
    setshowpop(false)
  };
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!harmlessagreement.name || !harmlessagreement.initials || !agreed || !harmlessagreement.signatureurl) {
  ...
  return (
    ...
    <div className="flex flex-col flex-1 p-2 rounded-md gap-2 justify-between bg-gray-800">
      ...
      <div className="flex gap-1 items-center justify-center">
          ...
          <div className="flex gap-1 items-center justify-center">
            <input type="checkbox" checked={agreed} onChange={handleAgreementToggle} />
          ...
          {showpop && agreed && harmlessagreement.initials && (
            <div className="w-full flex justify-center items-center">
              <SignatureCanvas
                penColor="black"
                canvasProps={{ width: 300, height: 150, className: 'sigCanvas',style: { border: '1px solid #000', backgroundColor: '#9ca3af'  , borderRadius : "10px"} }}
                ref={signatureRef}
              />

<button type="button" className='mx-2 ' style={{ background: '#e74c3c', color: 'white', padding: '8px', borderRadius: '4px', border: 'none', cursor: 'pointer',  }} onClick={handleClear}>{t('Clear')}</button>
              <button
